...
for ($row = 0; $row < count($numbers); $row++) {
    for ($col = 0; $col < count($numbers[$col])-3; $col++) {
        $n = $numbers[$row][$col] * $numbers[$row][$col+1] * $numbers[$row][$col+2] * $numbers[$row][$col+3];
        if ($n > $largest) {
            $largest = $n;
        }
    }
...
// Vertical
for ($col = 0; $col < count($numbers); $col++) {
    for ($row = 0; $row < count($numbers)-3; $row++) {
        $n = $numbers[$row][$col] * $numbers[$row+1][$col] * $numbers[$row+2][$col] * $numbers[$row+3][$col];
        if ($n > $largest) {
            $largest = $n;
        }
    }
...
// Diagonal forward
for ($row = 3; $row < count($numbers); $row++) {
    for ($col = 0; $col < count($numbers[$row])-3; $col++) {
        $n = $numbers[$row][$col] * $numbers[$row-1][$col+1] * $numbers[$row-2][$col+2] * $numbers[$row-3][$col+3];
        if ($n > $largest) {
            $largest = $n;
        }
    }
